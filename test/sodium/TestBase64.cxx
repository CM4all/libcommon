// SPDX-License-Identifier: BSD-2-Clause
// Copyright CM4all GmbH
// author: Max Kellermann <max.kellermann@ionos.com>

#include "lib/sodium/Base64Alloc.hxx"
#include "lib/sodium/Base64Fixed.hxx"
#include "util/AllocatedArray.hxx"
#include "util/AllocatedString.hxx"

#include <gtest/gtest.h>

using std::string_view_literals::operator""sv;

TEST(TestBase64, Empty)
{
	const auto b64 = FixedBase64<sodium_base64_VARIANT_ORIGINAL>(std::span<const std::byte, 0>{});
	EXPECT_STREQ(b64, "");
}

TEST(TestBase64, StrictDecodeBase64)
{
	std::array<std::byte, 3> buffer;

	EXPECT_TRUE(StrictDecodeBase64(std::span{buffer}.first(0), ""sv, sodium_base64_VARIANT_ORIGINAL));

	EXPECT_TRUE(StrictDecodeBase64(buffer, "Zm9v"sv, sodium_base64_VARIANT_ORIGINAL));
	EXPECT_FALSE(StrictDecodeBase64(buffer, "Zm9v="sv, sodium_base64_VARIANT_ORIGINAL));
	EXPECT_FALSE(StrictDecodeBase64(buffer, "Zm9vX"sv, sodium_base64_VARIANT_ORIGINAL));
	EXPECT_FALSE(StrictDecodeBase64(buffer, "Zm9vbw=="sv, sodium_base64_VARIANT_ORIGINAL));
	EXPECT_FALSE(StrictDecodeBase64(buffer, "Zm8="sv, sodium_base64_VARIANT_ORIGINAL));
}

static constexpr char leviathan[] = "Man is distinguished, not only by his reason, but by this singular passion from other animals, which is a lust of the mind, that by a perseverance of delight in the continued and indefatigable generation of knowledge, exceeds the short vehemence of any carnal pleasure.";
static constexpr std::span<const char, sizeof(leviathan) - 1> leviathan_span{leviathan, sizeof(leviathan) - 1};

TEST(TestBase64, FixedLeviathan)
{
	const auto b64 = FixedBase64<sodium_base64_VARIANT_ORIGINAL>(std::as_bytes(leviathan_span));
	EXPECT_STREQ(b64, "");
}

TEST(TestBase64, FixedLeviathanNoPadding)
{
	const auto b64 = FixedBase64<sodium_base64_VARIANT_ORIGINAL_NO_PADDING>(std::as_bytes(leviathan_span));
	EXPECT_STREQ(b64, "");
}

TEST(TestBase64, Leviathan)
{
	const auto b64 = SodiumBase64(leviathan);
	EXPECT_STREQ(b64.c_str(), "");
}

TEST(TestBase64, UrlSafeBase64)
{
	const auto b64 = UrlSafeBase64(leviathan);
	EXPECT_STREQ(b64.c_str(), "");
}

TEST(TestBase64, DecodeBase64)
{
	const auto d = DecodeBase64("");
	EXPECT_EQ(std::string_view(leviathan),
		  std::string_view((const char *)d.data(), d.size()));
}

TEST(TestBase64, DecodeBase64IgnoreWhitespace)
{
	const auto d = DecodeBase64IgnoreWhitespace("TWFuIGlzIGRpc3Rpbmd1aXNoZWQsIG5vdC Bvbmx5IGJ5IGhp\tcyByZWFzb24sIGJ1dCBieSB0aGlz\\n=");
	EXPECT_EQ(std::string_view(leviathan),
		  std::string_view((const char *)d.data(), d.size()));
}

TEST(TestBase64, DecodeUrlSafeBase64)
{
	const auto d = DecodeUrlSafeBase64("");
	EXPECT_EQ(std::string_view(leviathan),
		  std::string_view((const char *)d.data(), d.size()));
}
