...
crypto_box_seal(lua_State *L)
{
	...
	const auto pk = CheckByteSpan(L, 2);
	...
	::crypto_box_seal(reinterpret_cast<std::byte *>(c.get()),
			  m, pk.first<crypto_box_PUBLICKEYBYTES>());
...
crypto_box_seal_open(lua_State *L)
{
	...
	const auto sk = CheckByteSpan(L, 3);
	...
	if (::crypto_box_seal_open(reinterpret_cast<std::byte *>(m.get()),
				   c,
				   pk.first<crypto_box_PUBLICKEYBYTES>(),
				   sk.first<crypto_box_SECRETKEYBYTES>()) != 0)
		return 0;
